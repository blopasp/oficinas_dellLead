...
# valida as opcoes dentro da faixa de valores
def valida_opcao(opcao, max, min):
    return min <= opcao <= max

# a funcao compara as jogadas e retorna True caso o jogador_1 tenha ganhado
...
# Funcao usada para manipular o historico de jogadas do usuario. 
# O indice da lista coincide com o indice da lista de jogadas
def contagem(lista, jogada):
    if jogada == 1:
        lista[0] += 1
        return lista
    elif jogada == 2:
...
# Funcao utilizada para realizar realizar a aposta da maquina
# caso o limite seja 5, a funcao retornarar uma jogada aleatoria da maquina
# caso todos os valores sejam igual, a maquina retornara um valor aleatorio
# caso o contador seja maior ou igual a 5, a funcao retornara o elemento 
# que ganha do elemento que o usuario mais digitou
def zeroR(lista, jogadas, contador, limite = 5):
    if contador < limite:
    ...
    for i in range(len(lista)):
        if i != (len(lista) - 1):
            for j in range(len(lista)-1, i, -1):
                if (lista[i] > lista[j]) and (lista[i] > maior):
                    maior = lista[i]
                elif lista[j] > maior:
                    maior = lista[j]
    
    if jogadas[lista.index(maior)] == 'Pedra':
        jogada_maquina = 'Papel'
    elif jogadas[lista.index(maior)] == 'Papel':
        jogada_maquina = 'Tesoura'
    elif jogadas[lista.index(maior)] == 'Tesoura':
        jogada_maquina = 'Pedra'
    
    return jogada_maquina, jogadas.index(jogada_maquina) 
...
if __name__ == '__main__':
    ...
    menu = """
        ...
        4-Sair
    """
    # Adicionando historico de jogadas
    historico_jogadas = [0, 0, 0]

    a = 0
    while True:
        print(menu)

        jogada_maquina, index_maquina = zeroR(historico_jogadas, jogadas, a)
        
        try:
            jogada_usuario = int(input())
            historico_jogadas = contagem(historico_jogadas, jogada_usuario)
        except: 
